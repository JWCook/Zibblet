...
public enum CookingUnit {
    ...
    @EnumValue("ts") TEASPOON   ("tsp",     4.9289,     US, VOLUME),
    @EnumValue("tb") TABLESPOON ("tbsp",    14.7868,    US, VOLUME),
    @EnumValue("fl") FLUID_OUNCE("fl oz",   29.5735,    US, VOLUME),
    @EnumValue("c")  CUP        ("c",       236.5882,   US, VOLUME),
    @EnumValue("pt") PINT       ("pt",      473.1765,   US, VOLUME),
    @EnumValue("qt") QUART      ("qt",      946.3529,   US, VOLUME),
    @EnumValue("gl") GALLON     ("gal",     3785.4118,  US, VOLUME);
